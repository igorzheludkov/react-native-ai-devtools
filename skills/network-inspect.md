...
- Use `mcp__rn-ai-devtools__get_network_requests` with `summary=true` to get counts by method, status, and domain
- Alternatively, use `mcp__rn-ai-devtools__get_network_stats` for a quick stats overview
...
**By URL pattern:**
- Use `mcp__rn-ai-devtools__search_network` with `urlPattern` to find requests to specific endpoints
- Use `mcp__rn-ai-devtools__get_network_requests` with `urlPattern` filter

**By HTTP method:**
- Use `mcp__rn-ai-devtools__get_network_requests` with `method` filter (GET, POST, PUT, DELETE)

**By status code:**
- Use `mcp__rn-ai-devtools__get_network_requests` with `status` filter (e.g., 401, 500)

### 4. Inspect Request Details

For specific requests that need deeper investigation:
- Use `mcp__rn-ai-devtools__get_request_details` with the `requestId` from the list
...
- Use `mcp__rn-ai-devtools__clear_network` to reset the request buffer
...
- `mcp__rn-ai-devtools__get_network_requests`
- `mcp__rn-ai-devtools__get_network_stats`
- `mcp__rn-ai-devtools__search_network`
- `mcp__rn-ai-devtools__get_request_details`
- `mcp__rn-ai-devtools__clear_network`
